# Design Document: Predicting Answer Features Before the Model Speaks

## 1. Purpose

This document is the method contract for **Predicting Answer Features Before the Model Speaks**. It specifies what is
...
One-liner: Train maps from context activations to upcoming answer-feature probes.

Hypothesis: Train maps from context activations to upcoming answer-feature probes.
...
|---|---|---|
| Config schema | `ctxmap.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `ctxmap.configs.loader` | Hydra compose API |
| Device | `ctxmap.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `ctxmap.models.registry` | Pinned revisions + hardware fit |
| Hooks | `ctxmap.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `ctxmap.models.generation` | Chat template path recording |
| Data | `ctxmap.data` | Synthetic items, splits, manifests |
| Evaluation | `ctxmap.evaluation` | Metrics, failure harness, protocols |
| Ablation | `ctxmap.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `ctxmap.reporting` | Aggregate → MD/TeX/figures |
| Cache | `ctxmap.cache` | Versioned artifact store |
...
`src/ctxmap/ctxmap/` are preserved across regeneration.
